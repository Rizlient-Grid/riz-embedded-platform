...
timer_service::timer_service()
    : prev_ts_tick_ {tick_source_.now()} {}
...
void timer_service::submit_ms(std::uint32_t duration, timer_node& node) noexcept {
    assert(node.on_expire);
...
void timer_service::run() noexcept {
    ...
    std::uint32_t elapsed = now - prev_ts_tick_;
    delta_queue_.advance(elapsed, [](container::intrusive::delta_queue::node* node) {
